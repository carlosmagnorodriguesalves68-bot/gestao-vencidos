...
# state
if "faixas_sel_v117" not in st.session_state:
    st.session_state["faixas_sel_v117"] = [f[0] for f in FAIXAS]
if "status_manual_v117" not in st.session_state:
    st.session_state["status_manual_v117"] = {}
if "busca_v117" not in st.session_state:
    st.session_state["busca_v117"] = ""
if "nao_cobrados_v117" not in st.session_state:
    st.session_state["nao_cobrados_v117"] = False
...
with c3:
    busca = st.text_input("Buscar cliente", value=st.session_state["busca_v117"], placeholder="Buscar cliente", label_visibility="collapsed")
with c4:
    nao_cobrados = st.checkbox("Não cobrados", value=st.session_state["nao_cobrados_v117"])

st.session_state["busca_v117"] = busca
st.session_state["nao_cobrados_v117"] = nao_cobrados
...
for cliente in df["Cliente"].astype(str).unique():
    if cliente not in st.session_state["status_manual_v117"]:
        st.session_state["status_manual_v117"][cliente] = "Não cobrado"

df["Situação Manual"] = df["Cliente"].astype(str).map(st.session_state["status_manual_v117"])
...
clientes_df["Situação Manual"] = clientes_df["Cliente"].astype(str).map(st.session_state["status_manual_v117"])
...
for i, (nome, _ini, _fim, legenda, acao) in enumerate(FAIXAS):
    ...
    ativo = nome in st.session_state["faixas_sel_v117"]
    ...
    with btn_cols[i]:
        if st.button(label, key=f"faixa_{i}_{nome}", type="primary" if ativo else "secondary", use_container_width=True):
            selecionadas = list(st.session_state["faixas_sel_v117"])
            ...
            if len(selecionadas) == 0:
                selecionadas = [f[0] for f in FAIXAS]

            st.session_state["faixas_sel_v117"] = selecionadas
            st.rerun()

        st.markdown(f'<div class="legend-mini">{legenda}</div>', unsafe_allow_html=True)

faixas_ativas = st.session_state["faixas_sel_v117"]
...
with left:
    ...
    edited = st.data_editor(
        ...
        key="editor_checklist_v117"
    )

    for _, row in edited.iterrows():
        st.session_state["status_manual_v117"][str(row["Cliente"])] = row["Situação Manual"]

    filtrado["Situação Manual"] = filtrado["Cliente"].astype(str).map(st.session_state["status_manual_v117"])
