...
st.caption("Suba a planilha bruta do dia. O sistema aceita CSV e Excel, calcula os dias e monta a cobrança.")
...
if "status_manual_v10" not in st.session_state:
    st.session_state["status_manual_v10"] = {}

for _, row in df.iterrows():
    chave = row["chave"]
    if chave not in st.session_state["status_manual_v10"]:
        st.session_state["status_manual_v10"][chave] = "Não cobrado"

df["Situação Manual"] = df["chave"].map(st.session_state["status_manual_v10"])

f1, f2, f3 = st.columns([1.2, 1.4, 1.1])
with f1:
    status_sel = st.multiselect("Status", list(STATUS_ORDEM.keys()), default=list(STATUS_ORDEM.keys()))
with f2:
...
with left:
    ...
    edited = st.data_editor(
        ...
        key="editor_checklist_v10"
    )
    for _, row in edited.iterrows():
        st.session_state["status_manual_v10"][row["chave"]] = row["Situação Manual"]

filtrado["Situação Manual"] = filtrado["chave"].map(st.session_state["status_manual_v10"])
...
with right:
    ...
    clientes_msg = (
        ...
        .agg(Valor_total=("Montante", "sum"), Titulos=("N doc", "count"), Pior_ordem=("ordem", "min"))
