...
st.set_page_config(page_title="Gestão de Vencidos V11.5", layout="wide")
...
FAIXA_CORES = {
    "Recuperação de Perda": {"bg": "#d1d5db", "text": "#111827"},
    "Protesto Iminente": {"bg": "#fecaca", "text": "#991b1b"},
    "Radar de Perda": {"bg": "#fef3c7", "text": "#92400e"},
    "Bloqueio": {"bg": "#fed7aa", "text": "#9a3412"},
...
def faixa_por_dias(dias):
    for nome, ini, fim, _legenda, _acao in FAIXAS:
        if ini <= dias <= fim:
            return nome
...
.block-container { padding-top: 0.45rem; padding-bottom: 1.2rem; max-width: 1500px; }
h1, h2, h3 { margin-top: 0.2rem !important; margin-bottom: 0.35rem !important; }
div[data-testid="stVerticalBlock"] > div:has(> div[data-testid="stFileUploader"]) { margin-bottom: 0rem; }
...
.faixa-wrap { margin-top: 0.15rem; margin-bottom: 0.4rem; }
...
# state
if "faixas_sel_v115" not in st.session_state:
    st.session_state["faixas_sel_v115"] = [f[0] for f in FAIXAS]
if "status_manual_v115" not in st.session_state:
    st.session_state["status_manual_v115"] = {}
if "busca_v115" not in st.session_state:
    st.session_state["busca_v115"] = ""
if "nao_cobrados_v115" not in st.session_state:
    st.session_state["nao_cobrados_v115"] = False

st.markdown('<div class="header-title">📊 Gestão de Vencidos</div>', unsafe_allow_html=True)

c1, c2, c3, c4 = st.columns([1.25, 0.8, 1.2, 0.8])
with c1:
...
with c3:
    busca = st.text_input("Buscar cliente", value=st.session_state["busca_v115"], placeholder="Buscar cliente", label_visibility="collapsed")
with c4:
    nao_cobrados = st.checkbox("Não cobrados", value=st.session_state["nao_cobrados_v115"])

st.session_state["busca_v115"] = busca
st.session_state["nao_cobrados_v115"] = nao_cobrados
...
for cliente in df["Cliente"].astype(str).unique():
    if cliente not in st.session_state["status_manual_v115"]:
        st.session_state["status_manual_v115"][cliente] = "Não cobrado"
df["Situação Manual"] = df["Cliente"].astype(str).map(st.session_state["status_manual_v115"])
...
clientes_df["Situação Manual"] = clientes_df["Cliente"].astype(str).map(st.session_state["status_manual_v115"])
...
for i, (nome, _ini, _fim, legenda, acao) in enumerate(FAIXAS):
    ...
    ativo = nome in st.session_state["faixas_sel_v115"]
    ...
    with btn_cols[i]:
        if st.button(label, key=f"faixa_{i}_{nome}", type="primary" if ativo else "secondary", use_container_width=True):
            st.session_state["faixas_sel_v115"] = [nome]
            st.rerun()
        st.markdown(f'<div class="legend-mini">{legenda}</div>', unsafe_allow_html=True)

faixas_ativas = st.session_state["faixas_sel_v115"]
...
with left:
    ...
    edited = st.data_editor(
        ...
        key="editor_checklist_v115"
    )

    for _, row in edited.iterrows():
        st.session_state["status_manual_v115"][str(row["Cliente"])] = row["Situação Manual"]

    filtrado["Situação Manual"] = filtrado["Cliente"].astype(str).map(st.session_state["status_manual_v115"])
